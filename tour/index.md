...
| `time.duration` | time.duration(string) : parses a duration string (including eg 1d3h) and returns the duration in seconds
...
| `time.parse` | time.parse(string, [string]) : parses a time string (using optional format) and returns seconds since epoch (time is relative and after current time and local TZ if not specified by the format used)
